document.addEventListener('DOMContentLoaded', () => {
    ...
    function animate() {
        ...
        nodes.forEach(node => {
            ...
            nodes.forEach(otherNode => {
                ...
                if (distance < 100) {  // Aumentado para que se vean más conexiones
                    ctx.beginPath();
    ...
    // Cargar datos iniciales desde data.json si localStorage está vacío
    const entries = JSON.parse(localStorage.getItem('entries') || '[]');
    if (entries.length === 0) {
        fetch('data.json')
        ...
        entries.forEach(entry => addNode(entry));
    }
